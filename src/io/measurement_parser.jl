...
function get_sigma(σ::Float64, meas_var::String,phases)
    ...
    elseif meas_var in ["qd", "qtot", "qg"]
        sigma = σ*4
    elseif meas_var in ["crg", "cig", "crd_bus", "cid_bus"]   # fix
        sigma = σ
    else
        sigma = 0.02/3
    end
...
function write_measurements!(model::Type, data::Dict, pf_results::Dict, path::String; exclude::Vector{String}=String[], σ::Union{Dict, Float64}=0.005)
    ...
    for cmp_type in ["gen", "load"]
        ...
        write_cmp_measurements!(df, model, cmp_type, data, pf_results, very_basic_case, exclude = exclude, σ =σ) # fix , not σ = σ_cmp als dat er voor stond
    end
    _CSV.write(path, df)
